...
# Правила салона — раздел «📜 Правила салону» в контактах.
# ВАЖНО: формулировки предварительные, подтвердить у Анны перед показом.
SALON_RULES: Final[tuple[str, ...]] = (
    "Будь ласка, приходьте вчасно. За запізнення понад 15 хвилин "
    "майстер має право скоротити або перенести процедуру — далі за графіком інші клієнти.",
    "Якщо плани змінилися, скасуйте запис у боті щонайменше за 3 години. "
    "Так час дістанеться іншій клієнтці.",
    "На процедуру приходьте, будь ласка, без супроводу — у кабінеті працює одна людина.",
    "Якщо ви застудилися або погано почуваєтеся, краще перенести візит.",
    "Попередьте майстра про алергії, вагітність, цукровий діабет та інші особливості здоров'я — "
    "це впливає на вибір матеріалів і техніки.",
    "Корекція протягом 3 днів після процедури — безкоштовно, якщо це гарантійний випадок.",
    "Оплата — готівкою або переказом на картку після процедури.",
)
...
# --------------------------------------------------------------------------- #
# Возврат клиентов: напоминание о повторной записи
# --------------------------------------------------------------------------- #

# Через сколько дней после визита предложить записаться снова.
# Ключ — код категории услуг, значение — количество дней.
FOLLOWUP_DAYS: Final[dict[str, int]] = {
    "man": 21,   # маникюр — типичный интервал между коррекциями
    "ped": 35,   # педикюр — реже
    "pod": 42,   # подология — по курсу лечения
}

# В котором часу отправлять такие сообщения (не ночью и не в разгар работы).
FOLLOWUP_HOUR: Final[int] = 11

# Старше этого возраста визит считается «холодным» — не напоминаем,
# чтобы бот не написал клиентке про процедуру полугодовой давности.
FOLLOWUP_MAX_AGE_DAYS: Final[int] = 120

# Полностью выключить напоминания о повторной записи.
