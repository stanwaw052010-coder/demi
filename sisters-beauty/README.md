...
- **Анімацій рівно дві:** поява секцій під час скролу і мікровзаємодії при наведенні.
  Обидві вимикаються, якщо в системі увімкнено «зменшити рух» (`prefers-reduced-motion`).
...
3. **Важливо:** у полі **Root Directory** вкажіть `sisters-beauty` — сайт лежить у підпапці.
4. Framework Preset визначиться сам (Next.js). Build Command і Output залиште за замовчуванням.
5. Deploy. Далі: Settings → Domains → додайте свій домен і пропишіть DNS у реєстратора.
6. Після підключення домену замініть `url` у `src/lib/site.ts` і задеплойте ще раз.
