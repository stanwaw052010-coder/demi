...
def rejuvenation_types_keyboard() -> InlineKeyboardMarkup:
    """
    Лазерные процедуры омоложения.

    Фотоомоложение стоит отдельным пунктом меню, а не здесь: по нему от
    клиентки есть только цена, а этот список показывает карточки с курсом
    и интервалом. Раньше причина была другая — «это другая технология,
    IPL» — и она была ошибочной: IPL в студии нет.
    """
    builder = InlineKeyboardBuilder()
...
def request_rejuvenation_keyboard(selected: list[str] | None = None) -> InlineKeyboardMarkup:
    ...
    мультивыбор убран: любая процедура омоложения выбирается по одной.
    """
    builder = InlineKeyboardBuilder()
