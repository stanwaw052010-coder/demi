...
# ФОТООМОЛОДЖЕННЯ
# --------------------------------------------------------------------------- #
# ⚠️ ПРО IPL. Раніше ця послуга була описана в боті як «фотоомолодження
# (IPL)» — окрема технологія з власним апаратом, широкосмуговим імпульсним
# світлом. Це була помилка розробника: IPL у студії НЕМАЄ.
#
# Анна, 15.09.2026: «в нас нема IPL, в нас тільки фотоомолодження…
# треба IPL прибрати, оставити тільки лазерне омолодження».
#
# Тому: жодної згадки IPL, жодного «це інша технологія», жодного порівняння
# двох апаратів. Розділ омолодження один.
#
# Історія цін, щоб не плутатись:
#   10.09.2026 — Анна сказала, що послуги немає, її вимкнули.
#   14.09.2026 — повернула: «800 (база), 1500 повноцінний догляд
#                (кислотний пілінг, маска)».
#
# Тут лишилося ТІЛЬКИ те, що назвала клієнтка: назва, дві ціни й склад
# догляду. Опис механізму, тривалість, кількість сеансів та інтервал
# прибрані — їх вигадав розробник, і саме через такі вигадки й виникла
# плутанина з IPL. Повернемо, коли Анна скаже (CONTENT_TODO.md, пункт 12).
...
# Ні «sessions», ні «interval_days», ні «duration» тут немає навмисно:
# картка послуги показує тільки ті рядки, дані для яких існують.
_PHOTO_REJUV_ALL: Final[dict[str, Rejuvenation]] = {
    "r_photo_base": {
        "code": "r_photo_base",
        "name": "Фотоомолодження",
        "price": PHOTO_REJUV_PRICE_BASE,
        # 0 — пакета немає: клієнтка ціни за курс не називала.
        "package_price": 0,
    },
    "r_photo_care": {
        "code": "r_photo_care",
        "name": "Фотоомолодження + повноцінний догляд",
        "short": f"Процедура плюс догляд за обличчям: {PHOTO_REJUV_CARE_INCLUDES}.",
...
# Єдиний довідник омолодження. Окремої гілки для фотоомолодження більше
# немає — воно просто ще одна процедура в тому самому списку.
...
def is_photo_rejuvenation(code: str) -> bool:
    """Чи це фотоомолодження. Потрібно лише щоб приховати його разом з послугою."""
