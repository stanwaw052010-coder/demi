...
- **Анімації.** Сайт має шар руху, який вмикається сам і скрізь вимикається,
  ...
  **Важливо:** поява першого екрана зроблена на CSS, а не на JS — інакше на
  ...
  кількох секунд LCP.
- **Форма.** Заокруглення живуть однією шкалою в `globals.css` (`--r-xs` … `--r-pill`):
  картки, поля, фото і кнопки беруть радіус звідти, а не «на око». Портретні фото
  подані аркою (`.arch`) — це форма спа-ніші, вона тримає всю сторінку впізнаваною.
  Тіні теплі, коричнюваті (`--shadow-soft`, `--shadow-gold`): чиста чорна тінь на
  шоколадному тлі читається як бруд. Переходи між світлими і темними секціями
  вигнуті (`curveTop`), а не прямою лінією.
