...
Форма запису (`src/components/home/BookingForm.tsx`) без бекенду: після заповнення полів гість тисне «Написати у WhatsApp» або «Написати у Viber» — сайт:

1. Формує текст заявки (ім'я, телефон, послуга, дата, коментар).
2. Копіює його в буфер обміну.
3. Відкриває чат із салоном у відповідному месенджері (номер береться з `SITE.phoneHref` у `src/data/site.ts`) — для WhatsApp текст підставляється автоматично, для Viber його достатньо вставити з буфера.
4. Гостю залишається лише натиснути «Надіслати» в самому месенджері.

Нічого надсилати з сервера не треба — жодних API-ключів, акаунтів чи оплати. Щоб заявки приходили на інший номер, достатньо змінити `phone` / `phoneHref` в `src/data/site.ts`.
