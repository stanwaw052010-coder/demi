...
Форма запису (`src/components/home/BookingForm.tsx`) відправляє POST на `src/app/api/booking/route.ts`, який надсилає email на `spabelle.zp@gmail.com` через сервіс **Resend**. Щоб це запрацювало:

1. Зареєструватися на [resend.com](https://resend.com) (безкоштовно) і взяти API-ключ.
2. Покласти його в `.env.local` (не в `.env.example` — цей файл у git не потрапляє):
   ```
   RESEND_API_KEY=re_...
   ```
3. Перезапустити `npm run dev` / передеплоїти.

Без ключа форма поверне зрозумілу помилку користувачу («Сервіс надсилання листів тимчасово недоступний»), а не впаде мовчки.

Лист надсилається з тестової адреси Resend (`onboarding@resend.dev`) — вона працює одразу, без підтвердження власного домену, і листи доходять на будь-яку пошту, включно з Gmail. Якщо згодом захочете надсилати з власного домену (наприклад `booking@spabel.com.ua`), потрібно підтвердити домен у кабінеті Resend і замінити адресу `from` в `route.ts`.
