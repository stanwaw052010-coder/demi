...
`layout` вирішує, як показати пару:

- **`"slider"`** (за замовчуванням) накладає кадри один на одного —
  для цього вони мають бути зняті з однакового ракурсу;
- **`"split"`** ставить їх поруч. Саме так показано відбілювання: два кадри
  зняті під різними кутами, повзунок на них виглядав би зламано.
...
Секція 03 «Ціни» будується з `priceHighlights` (три великі картки згори)
і `priceGroups` (детальна таблиця) — п'ять груп, згрупованих за
