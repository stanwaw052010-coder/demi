...
Клієнтка просила «акуратну шапочку», з якої одразу видно, що це
стоматологія, — а не постер на весь екран. Тому `sections/hero.tsx` — це
візитка по центру: лок-ап, рядок «Сучасна стоматологія у Тернополі»,
адреса, телефон великим кеглем, графік і дві кнопки. Під нею — горизонтальна
стрічка фотографій, яка гортається вбік.

Дві речі там навмисні:

- блок з адресою й телефоном **не анімується**. Це найбільший елемент
  першого екрана, тож поява з прозорості відкладала б і показ номера, і
  метрику LCP;
...
| перший екран         | повний лок-ап 144 px по центру |
...
**mobile 89 / 100 / 100 / 100** (performance · accessibility · best practices · SEO).
Мобільний бал стрибає у межах 87–92 від прогону до прогону — це шум
середовища, а не збірки: вага сторінки тримається на 548 КБ.

Перший екран — компактна візитка (див. розділ «Перший екран»). Фото у
стрічці мають `fetchPriority="low"`: LCP тут текст, а стрічка починається
нижче згину, тож попереднє завантаження лише забирало б канал.
