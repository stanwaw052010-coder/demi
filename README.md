...
Hero → Про нас → Наші переваги → Наші послуги → Відгуки → Мобільний застосунок → Онлайн-запис (форма + WhatsApp/Telegram/телефон + графік роботи) → Як нас знайти (Google Maps) → CTA → Footer.
...
- **Фотографії** — обʼєкт `IMAGES` у `src/lib/site.ts` (зараз — стокові фото з Unsplash; щоб поставити власні, покладіть файли в `public/images/` і вкажіть локальний шлях, напр. `/images/hero.jpg`)
