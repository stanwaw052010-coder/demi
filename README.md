...
| Домен | змінна `NEXT_PUBLIC_SITE_URL` (за замовчуванням `https://www.profitime.center`) |
...
Детальний розбір — у **[SEO.md](./SEO.md)**: що зроблено в коді і що має
зробити студія в Google Business Profile. Коротко:

- **Канонічний домен** береться з `NEXT_PUBLIC_SITE_URL`, за замовчуванням
  `https://www.profitime.center`. Від нього залежать `canonical`, `sitemap.xml`,
  Open Graph і всі `url`/`@id` у мікророзмітці — тому значення має збігатися
  з реальним доменом, інакше Google бачить посилання на неіснуючу адресу.
- **Мікророзмітка**: `BeautySalon` і `FAQPage` на головній, `Service` +
  `BreadcrumbList` + `FAQPage` на кожній посадковій сторінці.
- **Що вирішує позиції в локальному пошуку** — не сайт, а Google Business
  Profile. Для запитів на кшталт «подолог Вишгород» Google спершу показує
  рекламу й блок карт, і лише потім органіку. Профіль має бути заповнений:
  сайт, телефон, години роботи, категорії, фото, відгуки.

### Посадкові сторінки

Односторінковий сайт може претендувати щонайбільше на один запит. Під решту
додані окремі сторінки — вміст у `src/lib/landing.ts`, маршрут один
(`src/app/[slug]/page.tsx`, статична генерація, `dynamicParams = false`).

| Адреса | Запит |
|---|---|
| `/podolog-vyshhorod` | подолог Вишгород |
| `/pedykiur-vyshhorod` | педикюр Вишгород |
| `/manikiur-vyshhorod` | манікюр Вишгород |
| `/vroslyi-nigot-vyshhorod` | врослий ніготь |

Ціни на них не дублюються, а беруться з `priceList` через `priceGroup()` /
`pickRows()`. Якщо групу або позицію в прайсі перейменують — білд впаде з
ясною помилкою замість тихо порожньої таблиці.

Щоб додати сторінку: новий об'єкт у `landingPages` — маршрут, `sitemap.xml`,
перелінковка в підвалі та мікророзмітка підхоплюються самі.
